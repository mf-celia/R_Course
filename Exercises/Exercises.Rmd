...
1. Load the data available in the file `retinol.txt`    
2. Create a histogram and a boxplot of `retplas` variable and plot them side-by-side on the same graphing region. Label the axes accordingly. Save your results as a Jpeg file.
3. Plot `retplas` (y-axis) versus `retdiet` (X-axis) using an appropriate plotting command. Put a title on the graph and
labels on the axes.
...
9. Create in one figure (e.g. use `par(mar=c(2,1))`) the histogram of variable `calorias` for males and females (variable `sexo`).
